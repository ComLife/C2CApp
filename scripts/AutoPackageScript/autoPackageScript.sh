...
environmentVariables() {
    ...
    project_dir=$(pwd)
    # 获取工程名称（LSBuyer）
    project_name=`find . -name $scheme_name.xcodeproj | awk -F "[/.]" '{print $(NF-1)}'`
...
#    read packageType
    sleep 0.5
    method="1" #"$packageType"

    # 判断用户是否有输入
    if [ -n "$method" ] ; then
        if [ "$method" = "1" ] ; then
            ExportOptionsPlistPath="$script_path/AdHocExportOptionsPlist.plist"
    ...
    # 获取对应的plist文件
    info_plist_path="$project_dir/ios/$project_name/$info_plist_name.plist"
...
# 上传文件到服务器生产环境下
uploadIPA(){
    remoteUrl="/home/devuser/app_download/$1/ios/$appName/"
    cd $script_path
    echo "\033[37;43m*************************  step5:373hao网站签名 🚀 🚀 🚀  *************************  \033[0m"
    sh updateIpa.sh $export_ipa_path/$ipa_name.ipa
    echo "\033[37;43m*************************  step6:修改PLIST文件 🚀 🚀 🚀  *************************  \033[0m"
    node 'fix-plist.js'
    echo "\033[37;43m*************************  step7:上传IPA包 🚀 🚀 🚀  *************************  \033[0m"
    ipa=$(find . -type f -name '*.ipa')
    "$script_path/../upload.sh" $ipa $remoteUrl
    echo "\033[37;43m*************************  step8:上传PLIST 🚀 🚀 🚀  *************************  \033[0m"
    "$script_path/../upload.sh" ../../$appName.plist $remoteUrl
    # 输出总用时
    echo "\033[37;46m总用时: ${SECONDS}s 👄 👄 👄 \033[0m"
}
